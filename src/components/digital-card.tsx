...
function AppIcon({ name }: { name: IconName }) {
  const common = { width: 28, height: 28, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 1.9, strokeLinecap: "round" as const, strokeLinejoin: "round" as const, "aria-hidden": true };
...
const driveLogoSources = ["https://lh3.googleusercontent.com/d/1DOneq1T0Hyi-xtbXrtSlX_ikvLq22Q9X", "https://drive.google.com/uc?export=view&id=1DOneq1T0Hyi-xtbXrtSlX_ikvLq22Q9X", "https://drive.google.com/thumbnail?id=1DOneq1T0Hyi-xtbXrtSlX_ikvLq22Q9X&sz=w1000"];
...
export function DigitalCard({ card }: { card: CardData }) {
  ...
  const hasEducation = Boolean(card.education || card.educationSecondary);
  ...
  useEffect(() => {
    ...
    const close = (e: KeyboardEvent) => {
      if (e.key === "Escape") { setQrOpen(false); setContactHelpOpen(false); }
    };
  ...
  async function saveContact() {
    ...
    if (/iPad|iPhone|iPod/.test(navigator.userAgent)) { window.location.assign(contactUrl); return; }
    setSaving(true);
    try {
      ...
      if (navigator.share && nav.canShare?.(shareData)) await navigator.share(shareData); else setContactHelpOpen(true);
    } catch (error) {
      if ((error as DOMException)?.name !== "AbortError") setContactHelpOpen(true);
  ...
  async function shareCard() {
    ...
    try {
      if (navigator.share) await navigator.share({ title: `${card.name} — ${card.company}`, text: card.role, url });
      else { await navigator.clipboard.writeText(url); setCopied(true); setTimeout(() => setCopied(false), 1600); }
    } catch (error) {
      if ((error as DOMException)?.name !== "AbortError") { await navigator.clipboard.writeText(url); setCopied(true); setTimeout(() => setCopied(false), 1600); }
    }
  }
      ...
      <article className="card-shell">
        <section className="hero">
          ...
          <header className="brand-row">
            <a href={card.website} target="_blank" rel="noreferrer" className="brand"><BrandLogo className="brand-mark" /><span><b>INFINITUM</b><small>NETWORK SOLUTIONS</small></span></a>
            <button className="icon-button" onClick={() => setQrOpen(true)} aria-label="Show QR code"><AppIcon name="qr" /></button>
          </header>
          <div className="profile-grid">
            <div className="photo-wrap"><img src={card.profileImage} alt={card.name} className="photo" loading="eager" referrerPolicy="no-referrer" /><span className="status"><i /> Executive Profile</span></div>
            <div className="identity">
              <p className="eyebrow">DIGITAL BUSINESS CARD</p><h1>{card.name}</h1><h2>{card.role}</h2><p className="company">{card.company}</p><p className="headline">{card.headline}</p>
              {hasEducation && <div className="education">{card.education && <span>{card.education}</span>}{card.educationSecondary && <span>{card.educationSecondary}</span>}</div>}
            </div>
          </div>
        </section>
        <section className="section"><p className="eyebrow">CONTACT DETAILS</p><h3>Connect directly</h3><div className="contact-list">
        ...
        <section className="panel social-panel"><h3>Connect Directly</h3><div className="social-list">{card.socialLinks.map((link) => <a key={link.platform} href={link.url} target="_blank" rel="noreferrer"><IconBox name={link.platform === "linkedin" ? "linkedin" : "instagram"} /><span><b>{link.label}</b><small>{link.handle}</small></span><em>›</em></a>)}</div></section>
        <section className="panel qr-panel"><p className="eyebrow">ONE SCAN. INSTANT ACCESS.</p><h3>Share this digital card</h3><p>Let anyone save {card.firstName}&apos;s details in seconds.</p><button onClick={() => setQrOpen(true)}><AppIcon name="qr" /> Show QR Code</button></section>
        <section className="bottom-actions" aria-label="Card actions"><div className="main-actions"><button type="button" onClick={saveContact} className="save-button"><AppIcon name="download" /><span>{saving ? "Opening…" : "Save Contact"}</span></button><button onClick={shareCard} className="share-button"><AppIcon name="share" />{copied ? "Link Copied" : "Share Card"}</button></div></section>
        <footer><BrandLogo alt="" /><span><b>INFINITUM</b><small>NETWORK SOLUTIONS</small></span><em>© 2026</em></footer>
      </article>
      {qrOpen && <div className="modal" onMouseDown={() => setQrOpen(false)}><div className="modal-card" onMouseDown={(e) => e.stopPropagation()}><button className="close" onClick={() => setQrOpen(false)}>×</button><p className="eyebrow">SCAN TO CONNECT</p><h3>{card.name}</h3><div className="qr-box"><QRCodeSVG value={typeof window === "undefined" ? `/${card.slug}` : window.location.href} size={230} level="H" marginSize={2} /></div><p>Point any phone camera at this QR code.</p></div></div>}
      {contactHelpOpen && <div className="modal" onMouseDown={() => setContactHelpOpen(false)}><div className="modal-card contact-help" onMouseDown={(e) => e.stopPropagation()}><button className="close" onClick={() => setContactHelpOpen(false)}>×</button><p className="eyebrow">SAVE TO CONTACTS</p><h3>One confirmation is required</h3><p>Android browsers do not allow a website to silently add someone to your phonebook. Download the contact card, open it from your Downloads notification, then choose <b>Contacts → Import</b>.</p><a className="contact-download" href={`/api/vcard/${card.slug}`} download={`${card.slug}.vcf`}><AppIcon name="download" /> Download Contact Card</a><button className="contact-cancel" onClick={() => setContactHelpOpen(false)}>Cancel</button></div></div>}
    </main>
  );
}
